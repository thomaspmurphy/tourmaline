...
import { fetchThread, createPost, clearCurrentThread, addPostToCurrentThread } from '../store/slices/threadsSlice'
...
const ThreadView: React.FC<ThreadViewProps> = ({ threadId, onBack }) => {
  ...
  const { currentThread, loading, error } = useSelector((state: RootState) => state.threads)
      ...
      <div className="bg-background border rounded-lg p-6 mb-8">
        ...
        <div className="flex items-center gap-6 text-sm text-muted-foreground border-t pt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{formatDistanceToNow(new Date(currentThread.created_at))} ago</span>
          </div>
          <div className="flex items-center gap-2">
            <MessageSquare className="w-4 h-4" />
            <span>
              {currentThread.posts?.length || 0} {(currentThread.posts?.length || 0) === 1 ? 'reply' : 'replies'}
            </span>
          </div>
        </div>
      </div>
        ...
        {isAuthenticated ? (
          <Card className="mt-8">
            <CardHeader>
              ...
              <CardDescription>Share your thoughts on this discussion</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="mt-8">
            <CardContent className="p-6 text-center">
              <p className="text-muted-foreground mb-4">You need to be logged in to reply</p>
